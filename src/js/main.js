(function($)
{
    // 接受<div id="danmup">Dom元素和相关配置dict，然后构造播放器实例
    var DanmuPlayer = function (element, options)
    {
        // 把Dom对象再转化成JQuery对象，方便使用
        ...
        this.sendDanmu = function(e)
        {
            ...
            {
                $.post(e.data.that.options.urlToPostDanmu, {danmu: textObj});
            }
    ...
    {
        // 这里的this.each里面所得到的this是一个个Dom对象
        return this.each(function()
        {
            ...
            // 如果还未初始化，则把该<div id="danmup">Dom元素和相关配置dict传进构造函数
            if(!data)
            {
    ...
    /*
     "jQuery.函数名 = 函数"可以扩展jQuery类本身.为类添加新的方法。
     jQuery.fn = jQuery.prototype，"jQuery.fn.函数名 = 函数"可以给jQuery对象实例添加方法
     */
