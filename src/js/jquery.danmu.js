...
(function()
{
    var $ = jQuery, pauseId = 'jQuery.pause', uuid = 1, oldAnimate = $.fn.animate, anims = {};
...
(function($)
{
    // 接受<div class="danmu-div">Dom元素和相关配置dict，然后构造弹幕层实例
                ...
                setTimeout(function()
                {
                    ...
                    if($(element).data("danmuList")[$(element).data("nowTime")] && me.launched.indexOf($(element).data("nowTime")) < 0)
                    {
                        ...
                        for(var i = (danmus.length - 1); i >= 0; i--)
                        {
                            ...
                            if(danmaku.size == 0)
                            {
                                $("#" + me.id + "tempDanmaku").css("font-size", options.fontSizeSmall);
                            }
    ...
    function Plugin(option, arg)
    {
        ...
        return this.each(function()
        {
            ...
            // 如果还未初始化，则把该<div class="danmu-div">Dom元素和相关配置dict传进构造函数
            if(!data)
            {
